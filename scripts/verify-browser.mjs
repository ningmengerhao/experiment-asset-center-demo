...
async function run() {
  ...
  try {
    ...
    await typeText("[data-create-custom-seed]", "custom_seed:01");
    await physicalClick("[data-validate-custom-seed]");
    await waitFor("custom seed validation", () => evaluate(`document.querySelector('[data-create-seed-value]')?.textContent === "custom_seed:01" || Boolean([...document.querySelectorAll('[data-create-seed-value]')].find((node) => node.textContent === "custom_seed:01"))`));
    ...
    assert.equal(await evaluate(`document.querySelector("[data-create-seed-candidate]:checked")?.value`), "custom_seed:01", "Validated custom seed must be selected automatically.");
    await physicalClick("[data-create-next]");
    ...
    assert.equal(await evaluate(`document.body.textContent.includes("新增首购引导") && document.body.textContent.includes("待上线") && Boolean(document.querySelector('[data-experiment-lifecycle="pending-running"]'))`), true, "Completing an edited draft must create a pending-launch record with an online action.");
