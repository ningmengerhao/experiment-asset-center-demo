...
普通用户从首页进入，再按实验阶段组织工作流：

1. 首页：实验清单、跨平台来源与当前状态概览。
2. 实验前：实验评估、样本量与周期测算。
3. 分流阶段：分流方案、随机数放量历史。
4. 上线前：Pre-AA、均匀性、正交性和规则冲突检查。
5. 运行中：告警中心、跨实验异常归因、告警规则配置和当前排查。
6. 实验管理：父子实验、放量历史和批量导入记录。

本版新增独立的四步“新增实验”向导：从首页的新建方式选择中进入，按实验基本信息、样本量评估、分流方案和校验结果完成登记。基本信息分别登记业务线和实验域；样本量评估支持多组分流比例，并以最小比例组推导总样本量与周期。分流方案只读复用这些配置，候选 seed 使用 4 至 8 位数字后缀；点击生成会最多自动刷新 20 轮，找到含“通过”结果的候选组即停止。点击保存会将当前信息作为“草稿”写入实验清单，可从清单编辑并继续；每次点击新建实验始终开启新的空白向导。向导不出现在侧边栏；草稿与完成后的本地记录保存在浏览器本机。既有实验评估、分流方案、随机数放量历史和上线前检查页面保持原有功能与入口。
...
- 使用 Git 分支、提交和 tag 管理可复现版本；`v0.1.0` 是导入基线，`v0.2.0` 是首页与清单筛选改版，`v0.2.1` 是清单筛选紧凑化，`v0.2.2` 是单行清单工具栏与新建方式选择，`v0.2.3` 是工具栏单行筛选与白色表面优化，`v0.3.0` 是本地草稿支持的四步新增实验向导，`v0.3.1` 是实验域、多组分流比例与可重复生成候选 seed 调整，`v0.3.2` 是短 seed 后缀与通过结果自动刷新，`v0.3.3` 是实验清单草稿和编辑恢复。
