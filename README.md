...
本版新增独立的四步“新增实验”向导：从首页的新建方式选择中进入，按实验基本信息、样本量评估、分流方案和校验结果完成登记。基本信息分别登记业务线和实验域；样本量评估支持多组分流比例，并以最小比例组推导总样本量与周期。分流方案只读复用这些配置，候选 seed 使用 4 至 8 位数字后缀；点击生成会最多自动刷新 20 轮，找到含“通过”结果的候选组即停止。向导不出现在侧边栏；草稿与完成后的本地记录保存在浏览器本机，刷新或再次打开仍可从最近一次保存的步骤继续。既有实验评估、分流方案、随机数放量历史和上线前检查页面保持原有功能与入口。
...
- 使用 Git 分支、提交和 tag 管理可复现版本；`v0.1.0` 是导入基线，`v0.2.0` 是首页与清单筛选改版，`v0.2.1` 是清单筛选紧凑化，`v0.2.2` 是单行清单工具栏与新建方式选择，`v0.2.3` 是工具栏单行筛选与白色表面优化，`v0.3.0` 是本地草稿支持的四步新增实验向导，`v0.3.1` 是实验域、多组分流比例与可重复生成候选 seed 调整，`v0.3.2` 是短 seed 后缀与通过结果自动刷新。
